...
namespace coding_test_ranking.infrastructure.persistence
{
    public class InMemoryPersistence
    {
        ...
        public InMemoryPersistence(IMapper mapper)
        {
            _mapper = mapper;
            ...
            ads.Add(new AdVO { Id = 8, Typology = "CHALET", Description = "Maravilloso chalet situado en lAs afueras de un pequeño pueblo rural. El entorno es espectacular, las vistas magníficas. ¡Cómprelo ahora!", Pictures = new List<int> { 1, 7 }, HouseSize = 300, GardenSize = 100 });
        ...
        {
            PictureVO sourcePicture = pictures.Find(p => p.Id == id);
        ...
        public Task UpdateAd(AdVO ad)
        {
            int itemIndex = ads.FindIndex(item => item.Id == ad.Id);
            if(itemIndex < 0)
            {
